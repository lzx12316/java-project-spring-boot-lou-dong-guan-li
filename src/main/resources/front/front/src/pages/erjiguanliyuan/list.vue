...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"border":"1px solid #C7C7C7","padding":"20px 10px 0","margin":"0 0 20px","alignItems":"center","background":"#fff","flexDirection":"row","display":"flex","width":"100%","height":"auto","order":"1"}'>
      <el-form-item :style='{"flexWrap":"wrap","margin":"0 0 20px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 5px 0","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>账号：</div>
	  ...
	  <el-button v-if="btnAuth('erjiguanliyuan','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 0 20px","outline":"none","color":"#fff","borderRadius":"0px","background":"#75664d","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}' type="primary" @click="add('/index/erjiguanliyuanAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'岗位',list:this.gangweiOptions,tableName: 'gangwei',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.gangwei != '') searchWhere.gangwei = this.formSearch.gangwei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`erjiguanliyuan/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/erjiguanliyuanDetail', query: params});
      },
