  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          fangyuanchengshi: [
            { required: true, message: '房源城市不能为空', trigger: 'blur' },
          ],
        },
		centerType: false,
      };
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`fangyuanchengshi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('fangyuanchengshi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`fangyuanchengshi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
