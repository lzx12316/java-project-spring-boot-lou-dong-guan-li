...
<div :style='{"width":"100%","padding":"20px 10%","margin":"20px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="月租/元" prop="yuezu">
			<el-input-number v-model="ruleForm.yuezu" placeholder="月租/元" :readonly="ro.yuezu"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="押金/元" prop="yajin">
			<el-input-number v-model="ruleForm.yajin" placeholder="押金/元" :readonly="ro.yajin"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="定金/元" prop="dingjin">
			<el-input-number v-model="ruleForm.dingjin" placeholder="定金/元" :readonly="ro.dingjin"></el-input-number>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="总金额/元" prop="zongjine">
              <el-input v-model="zongjine" placeholder="总金额/元" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="签订时间" prop="qiandingshijian">
              <el-date-picker
				  :disabled=" false  ||ro.qiandingshijian"
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.zuhuzhanghao!=''&&json.zuhuzhanghao) || json.zuhuzhanghao==0){
                this.ruleForm.zuhuzhanghao = json.zuhuzhanghao            }
            if((json.zuhuxingming!=''&&json.zuhuxingming) || json.zuhuxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zulinxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zulinxinxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zulinxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
