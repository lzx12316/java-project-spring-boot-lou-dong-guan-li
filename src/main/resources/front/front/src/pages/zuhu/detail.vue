...
<div>
	<div :style='{"padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.1)","margin":"0px auto","borderColor":"#75664D","background":"#fff","borderWidth":"5px 0 0 0","width":"80%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"center","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/zuhu"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"0 10% 20px","margin":"20px auto","flexWrap":"wrap","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<div class="attr" :style='{"borderColor":"#75664d","margin":"10px 2%","background":"#fff","flex":"1","borderWidth":"5px 0 0","display":"flex","width":"45%","position":"relative","borderStyle":"solid","order":"1"}'>
			...
			<div class="info" :style='{"background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"5px 10px","boxShadow":"0 1px 0 #D8D8D8","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"left","width":"auto","fontSize":"14px","lineHeight":"40px","height":"40px"}'>身份证</div>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#75664d","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('zuhu','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#75664d","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('zuhu','删除')" @click="delClick">删除</el-button>
  ...
  export default {
    ...
    methods: {
        init() {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              ...
              this.title = this.detail.zuhuxingming;
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/zuhuAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此租户？')
			  .then(_ => {
			    this.$http.post('zuhu/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
