...
<div :style='{"width":"100%","padding":"20px 10%","margin":"20px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}'  label="岗位" prop="gangwei">
            <el-select v-model="ruleForm.gangwei" placeholder="请选择岗位" :disabled=" false  ||ro.gangwei" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="请假时间" prop="qingjiashijian">
              <el-date-picker
				  :disabled=" false  ||ro.qingjiashijian"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="请假原因" prop="qingjiayuanyin">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="请假原因"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`qingjiashenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('qingjiashenqing/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`qingjiashenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
