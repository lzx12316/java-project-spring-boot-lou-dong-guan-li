		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"width":"80%","margin":"0 0 20px","alignItems":"flex-start","flexDirection":"column","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"1px solid #654B3C","width":"auto","overflow":"hidden","borderRadius":"20px","display":"inline-block"}' >
					...
					<div :style='{"margin":"0 0 0 0","display":"inline-block"}' class="select" label="岗位" prop="gangwei">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"none","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">岗位</label>
						<el-select clearable v-model="searchForm.gangwei" placeholder="请选择岗位" >
							<el-option v-for="(item,index) in gangweiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"width":"100%","margin":"20px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button class="add" v-if="isAuth('erjiguanliyuan','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#654B3C","height":"40px"}'></span>
						创建
					</el-button>
					<el-button class="del" v-if="isAuth('erjiguanliyuan','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px","boxShadow":""}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('erjiguanliyuan','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"14px","color":"#965F40","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('erjiguanliyuan','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('erjiguanliyuan','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    init () {
          this.gangweiOptions = "保洁,维修员".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
	...
	erjiguanliyuanstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'erjiguanliyuan/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
