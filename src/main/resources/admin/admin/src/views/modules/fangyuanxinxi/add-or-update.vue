	...
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="房源城市" prop="fangyuanchengshi">
					<el-select :disabled="ro.fangyuanchengshi" v-model="ruleForm.fangyuanchengshi" placeholder="请选择房源城市" >
						<el-option
							v-for="(item,index) in fangyuanchengshiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="房源城市" prop="fangyuanchengshi">
					<el-input v-model="ruleForm.fangyuanchengshi"
						placeholder="房源城市" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="朝向" prop="chaoxiang">
					<el-input v-model="ruleForm.chaoxiang" placeholder="朝向" clearable  :readonly="ro.chaoxiang"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="朝向" prop="chaoxiang">
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="房源类型" prop="fangyuanleixing">
					<el-select :disabled="ro.fangyuanleixing" v-model="ruleForm.fangyuanleixing" placeholder="请选择房源类型" >
						<el-option
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="配套详情" prop="peitaoxiangqing">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.peitaoxiangqing" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else-if="ruleForm.peitaoxiangqing" label="配套详情" prop="peitaoxiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","minWidth":"400px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.peitaoxiangqing"></span>
                </el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='peitaoxiangqing'){
							this.ruleForm.peitaoxiangqing = obj[o];
							this.ro.peitaoxiangqing = true;
							continue;
						}
						if(o=='clicktime'){
            ...
            this.$http({
				url: `option/fangyuanchengshi/fangyuanchengshi`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.fangyuanchengshiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.peitaoxiangqing = this.ruleForm.peitaoxiangqing.replace(reg,'../../../springboot4y8k8m47/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
